...
<template>
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-ink-900">{{ $t('dashboard.adminTitle') }}</h1>
        <p class="text-ink-500">{{ $t('dashboard.adminSubtitle') }}</p>
        ...
        <template v-else>
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <StatCard :label="$t('dashboard.statCourses')" :value="data.courses_count" icon="book" tone="sky" />
                <StatCard :label="$t('dashboard.statCompetitions')" :value="data.competitions_count" icon="trophy" tone="amber" />
            </div>
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard :label="$t('dashboard.activeEnrollments')" :value="data.active_enrollments_count" icon="layers" tone="ink" />
            ...
            <AppCard :title="$t('dashboard.integritySignal')">
                <div class="flex items-center justify-between rounded-lg bg-rose-50 px-5 py-4">
                    <div>
                        <p class="font-semibold text-rose-800">{{ $t('dashboard.flaggedIntegrity') }}</p>
                        <p class="text-sm text-rose-600">{{ data.flagged_integrity_count }} {{ $t('dashboard.flaggedAttempts') }}</p>
                    </div>
