    ...
    <div class="space-y-6">
        <div>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-3">
                <h1 class="text-2xl font-bold text-ink-900">{{ data?.title || 'Competition' }}</h1>
                <AppBadge :tone="data?.status === 'active' ? 'success' : data?.status === 'published' ? 'primary' : 'neutral'">{{ data?.status }}</AppBadge>
                <router-link :to="`/teacher/competitions/${id}/edit`"><AppButton variant="outline" size="sm">Edit</AppButton></router-link>
            </div>
            <p class="mt-1 text-ink-500">{{ data?.description }}</p>
        </div>

        <div class="flex flex-wrap gap-2">
            <AppButton variant="outline" size="sm" :loading="recalcBusy" @click="recalc">Recalculate leaderboard</AppButton>
        </div>
        ...
        <template v-else>
            <AppCard title="Participants">
                <EmptyState v-if="!participants.length" icon="users" title="No participants yet" message="Students who join the competition will appear here." />
                <div v-else class="space-y-2">
                    <div v-for="p in participants" :key="p.id" class="flex items-center gap-3 rounded-lg border border-ink-100 px-4 py-3">
                        ...
                        <span class="min-w-0 flex-1 truncate font-medium text-ink-800">{{ p.student_display_name }}</span>
                        <AppBadge :tone="p.status === 'disqualified' ? 'danger' : p.status === 'qualified' ? 'success' : 'neutral'">{{ p.status }}</AppBadge>
                        <button class="rounded-lg px-2.5 py-1.5 text-xs font-medium text-rose-600 hover:bg-rose-50" @click="disqualify(p)">Disqualify</button>
                    </div>
            ...
            <AppCard title="Leaderboard">
                <EmptyState v-if="!leaderboard.length" icon="trophy" title="No rankings yet" message="Rankings appear once participants submit the linked exam." />
                <div v-else class="space-y-2">
                    <div v-for="row in leaderboard" :key="row.rank + row.student_display_name" class="flex items-center gap-3 rounded-lg px-4 py-2.5" :class="row.rank <= 3 ? 'bg-parchment-100' : 'bg-ink-50/50'">
                        ...
                        <AppBadge v-if="row.qualified !== undefined" :tone="row.qualified ? 'success' : 'neutral'">{{ row.qualified ? 'Qualified' : '—' }}</AppBadge>
                    </div>
