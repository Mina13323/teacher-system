...
<template>
    <div class="space-y-6">
        <div>
            <router-link to="/teacher/analytics" class="text-sm font-medium text-terracotta-600 hover:underline">← {{ $t('analytics.backToAnalytics') }}</router-link>
            <h1 class="mt-2 text-2xl font-bold text-ink-900" dir="auto">{{ data?.student?.name || $t('analytics.fallbackTitle') }}</h1>
        ...
        <template v-else>
            ...
            <AppCard :title="$t('analytics.examHistory')">
                <EmptyState v-if="!data.history?.length" icon="clipboard" :title="$t('analytics.noHistoryStudentTitle')" :message="$t('analytics.noHistoryStudentMessage')" />
