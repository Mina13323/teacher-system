...
<template>
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-ink-900">Integrity review</h1>
        ...
        <template v-else>
            <StatCard label="Attempts flagged / monitoring" :value="data.flagged_integrity_count" icon="shield" tone="danger" />

            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-ink-900">Attempts needing attention</h2>
                <AppButton variant="outline" :loading="collecting" @click="collect">Refresh</AppButton>
            </div>

            <LoadingSpinner v-if="collecting" />
            <EmptyState v-else-if="!flagged.length" icon="shield" title="No flagged attempts" message="Attempts with an integrity status will appear here for review." />
            <div v-else class="space-y-3">
                <div v-for="a in flagged" :key="a.id" class="flex items-center gap-3 rounded-xl border border-ink-100 bg-white p-4 shadow-sm">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="font-medium text-ink-800">{{ a.student?.name }} · {{ a.exam_title }}</p>
                        <p class="text-xs text-ink-400">{{ a.course_title }} · Attempt {{ a.attempt_number }} · {{ a.percentage !== null ? a.percentage + '%' : '—' }}</p>
                    </div>
                    <AppBadge :tone="tone(a.integrity_status)">{{ a.integrity_status }}</AppBadge>
                    <router-link :to="`/teacher/integrity/attempts/${a.id}`"><AppButton size="sm">Review</AppButton></router-link>
                </div>
            </div>
        </template>
    </div>
