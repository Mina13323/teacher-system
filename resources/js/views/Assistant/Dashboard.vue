...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-ink-900">Assistant dashboard</h1>
                <p class="text-sm text-ink-500">Manage student accounts and course enrollments.</p>
            </div>
            <router-link to="/assistant/students/new"><AppButton>Add student</AppButton></router-link>
        </div>
        ...
        <template v-else>
            <div class="grid gap-4 sm:grid-cols-3">
                <StatCard label="Students" :value="meta?.total ?? students.length" icon="users" tone="terracotta" />
                <StatCard label="Active students" :value="students.filter((s) => s.is_active).length + (meta && meta.total > students.length ? '+' : '')" icon="user" tone="emerald" />
                <StatCard label="This page" :value="students.length" icon="users" tone="sky" />
            </div>

            <div class="overflow-hidden rounded-xl border border-ink-100 bg-white shadow-sm">
                <div class="px-5 py-4"><h2 class="font-semibold text-ink-900">Recent students</h2></div>
                <EmptyState v-if="!students.length" icon="users" title="No students yet" message="Create a student account to get started.">
                    <router-link to="/assistant/students/new"><AppButton>Add student</AppButton></router-link>
                </EmptyState>
