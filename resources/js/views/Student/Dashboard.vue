    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-ink-500">Continue your learning journey.</p>
            </div>
            <router-link to="/student/courses"><AppButton>Browse courses</AppButton></router-link>
        </div>
        ...
        <template v-else>
            <div class="grid gap-4 sm:grid-cols-3">
                <StatCard label="Enrolled courses" :value="data.enrolled_courses_count" icon="book" tone="terracotta" />
                <StatCard label="Completed lessons" :value="data.completed_lessons_count" icon="check" tone="emerald" />
                <StatCard label="In-progress lessons" :value="data.in_progress_lessons_count" icon="layers" tone="sky" />
            </div>

            <div v-if="data.recently_accessed_lessons.length" class="overflow-hidden rounded-xl border border-ink-100 bg-white shadow-sm">
                <div class="px-5 py-4"><h2 class="font-semibold text-ink-900">Continue where you left off</h2></div>
                <div class="divide-y divide-ink-100">
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="truncate font-medium text-ink-800">{{ l.lesson?.title || `Lesson #${l.lesson_id}` }}</p>
                            <p class="text-xs text-ink-400">{{ l.progress_percentage }}% complete · {{ fmtDate(l.updated_at) }}</p>
                        </div>
                        <span class="text-xs font-medium text-terracotta-600">Resume</span>
                    </router-link>
                </div>
            </div>

            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-ink-900">My courses</h2>
                <router-link to="/student/courses" class="text-sm font-medium text-terracotta-600 hover:underline">View all</router-link>
            </div>
            ...
            <EmptyState
                ...
                message="Browse the catalog and enroll to get started."
            >
