...
<template>
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-ink-900">{{ $t('nav.exams') }}</h1>
        ...
        <EmptyState v-else-if="!data.items.length" icon="clipboard" :title="$t('exams.emptyTitle')" :message="$t('exams.emptyMessage')" />
            ...
            <router-link v-for="e in data.items" :key="e.id" :to="`/student/exams/${e.id}`" class="group block">
                <div class="flex items-center gap-4 rounded-xl border border-ink-100 bg-white p-5 shadow-sm transition group-hover:shadow-md">
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="font-semibold text-ink-900 group-hover:text-terracotta-700" dir="auto">{{ e.title }}</h3>
                        <p class="truncate text-sm text-ink-500" dir="auto">{{ e.course?.title }}</p>
                    </div>
                    <div class="hidden items-center gap-3 text-xs text-ink-400 sm:flex">
                        <span>{{ e.duration_minutes }} {{ $t('common.minutesShort') }}</span>
                        <span class="h-1 w-1 rounded-full bg-ink-300" />
                        <span>{{ $t('common.questionsCount', { n: e.questions_count }) }}</span>
                        <span class="h-1 w-1 rounded-full bg-ink-300" />
                        <span>{{ $t('common.attemptsCount', { n: e.max_attempts }) }}</span>
                    </div>
                    <AppBadge tone="primary">{{ $t('exams.takeExam') }}</AppBadge>
                </div>
            </router-link>
