...
<template>
    <div class="min-h-screen bg-parchment-50">
        <header class="sticky top-0 z-20 border-b border-ink-100 bg-parchment-50/95 pt-safe backdrop-blur">
            <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-3 sm:px-4">
                ...
                <nav class="flex items-center gap-2 sm:gap-3 shrink-0">
                    ...
                    <router-link to="/" class="text-xs sm:text-sm font-medium text-ink-600 hover:text-ink-900">{{ $t('app.home') }}</router-link>
                    <router-link to="/login" class="text-xs sm:text-sm font-medium text-ink-600 hover:text-ink-900">{{ $t('auth.signIn') }}</router-link>
                </nav>
            </div>
        </header>
